...
pub(crate) fn date_time_string_to_datetime(date: &str, time: &str) -> Result<DateTime<FixedOffset>, DateTimeError> {
    let date_time = DateTime::parse_from_str(&format!("{} {} +02:00", date, time), "%d.%m.%Y %H:%M:%S %z").map_err(|e| DateTimeError::DateTimeInvalid(format!("converting '{} {}') +02:00 failed with error '{}'", date, time, e)))?;
    Ok(date_time)
}
...
pub(crate) fn merge_naive_date_time_to_datetime(date: &NaiveDate, time: &NaiveTime) -> Result<DateTime<FixedOffset>, DateTimeError> {
    let date_time = DateTime::parse_from_str(&format!("{} {} +02:00", date, time), "%Y-%m-%d %H:%M:%S %z").map_err(|e| DateTimeError::DateTimeInvalid(format!("merging '{} {}' +02:00 failed with error '{}'", date, time, e)))?;
    Ok(date_time)
}